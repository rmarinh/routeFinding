...
EXCEPTION
    ...
    WHEN excep_encomendainexistente THEN
        raise_application_error(-20003, 'Não existe encomenda em sistema');
END;
/
...
EXCEPTION
    ...
    WHEN excep_MochilasInexistente THEN
        raise_application_error(-20003, 'Não existem mochilas no sistema');
...
EXCEPTION
    ...
    WHEN excep_ruaInexistente THEN
        raise_application_error(-20003, 'Pelo menos um dos enderecos é inexistente em sistema');
    WHEN excep_RuasIguais THEN
        raise_application_error(-20003, 'Endereços indicados são iguais');
END;
/
...
EXCEPTION
    ...
    WHEN excep_parametroInvalido THEN
        raise_application_error(-20002, 'Parâmetros inválidos');

    WHEN excep_enderecoInexistentes THEN
        raise_application_error(-20003, 'Endereco inserido é desconhecido em sistema');
...
BEGIN
    ...
    IF (p_idFarmacia IS NOT NULL) THEN
        ...
        -- se existe:
        IF (v_existeFarmacia) THEN
            -- verificar se há scooters disponiveis:
...
EXCEPTION
    ...
    WHEN excep_naoExisteScooterDisponiveis THEN
        raise_application_error(-20001, 'Não existe scooters disponiveis');
        RETURN 0;
...
BEGIN
    ...
    IF (p_idFarmacia IS NOT NULL) THEN
        ...
        -- se existe:
        IF (v_existeFarmacia) THEN
            -- verificar se há drones disponiveis:
            SELECT COUNT(a.id_veiculo)
            INTO v_existeVeiculo
            FROM (SELECT id_veiculo
                  FROM veiculo
...
EXCEPTION
    ...
    WHEN excep_naoExisteDroneDisponiveis THEN
        raise_application_error(-20001, 'Não existe drone disponivel');
        RETURN 0;
    WHEN excep_naoExisteFarmacia THEN
        raise_application_error(-20001, 'Farmacia desconhecida em sistema');
        RETURN 0;
