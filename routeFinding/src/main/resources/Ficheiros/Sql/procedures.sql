            ...
            IF (v_farmaciaExiste) THEN
                ...
                IF (v_parqueExiste) THEN
                    ...
                    IF (v_numVeiculoPorFarmacia < v_numTotalEstacionamento) THEN
                        ...
                        -- Get id do veiculo mais recente (veiculo criado anteriormente)
                        ...
                        SELECT DISTINCT a.id_lugarEstacionamento
                        INTO v_lugarParaEstacionar
                        FROM (SELECT * FROM lugar_estacionamento WHERE id_parque = v_idParque) a
                                 LEFT JOIN r_veiculo_lugarEstacionamento
                                           ON r_veiculo_lugarEstacionamento.id_lugarEstacionamento =
                                              a.id_lugarEstacionamento
...
-- Caso tenha faturas, get ultima fatura
    ...
    IF p_gastarCreditos = 1 THEN
        ...
        -- Cria fatura com valor de creditos a descontar
        INSERT INTO FATURA (NUMERO, DATA, ID_ENCOMENDA, VALORFATURADO, VALORTRANSPORTE, VALORDESCONTADO)
        VALUES (nrFatura, sysdate, p_idEncomenda, totalValorFaturado, transporte, valorCreditos);
...
create PROCEDURE prcInserirArtigo(p_designacao artigo.designacao%type, p_precoUnitario artigo.precoUnitario%type,
...
create PROCEDURE prcAtualizarCliente(p_nif cliente.NIF%type, p_nome cliente.nome%type,
                                     p_password conta.PASSWORD%type, p_rua LOCALIDADE.DESIGNACAO%type,
                                     p_latitude LOCALIDADE.LATITUDE%type,
                                     p_longitude LOCALIDADE.LONGITUDE%type,
                                     p_email cliente.EMAIL%type, p_cartaoCredito cliente.CARTAOCREDITO%type)
...
create PROCEDURE prcRegistarDrone(p_nrRegisto drone.NUMREGISTO%type,
                                  p_idFarmacia farmacia.ID_FARMACIA%type)
AS
    tipoVeiculo          integer := 2; -- tipo de veiculo -> drone
    idModelo             integer := 1; -- há apenas um modelo de drone
    idTipoBateria        integer := 2; -- há apenas um tipo de bateria
    idEstado             integer := 1; -- por defeito, o estado é 'funcional'
    cargaBateriaAtual    integer := 100; -- por defeito, a carga da bateria atual = 100%
    idVeiculoMaisRecente veiculo.ID_VEICULO%type;

BEGIN

    -- Criar o veiculo
    prcRegistarVeiculo(tipoVeiculo, p_idFarmacia);

    -- Get id mais recente (veiculo criado anteriormente)
    SELECT MAX(ID_VEICULO) into idVeiculoMaisRecente FROM veiculo;

    -- Criar drone
    Insert Into DRONE (ID_DRONE, NUMREGISTO, ID_MODELO, ID_TIPOBATERIA, ID_ESTADO, CAPACIDADE_ATUAL)
    VALUES (idVeiculoMaisRecente, p_nrRegisto, idModelo, idTipoBateria, idEstado, cargaBateriaAtual);

END;
/

create PROCEDURE prcAtualizarDrone(p_nrRegisto drone.NUMREGISTO%type,
                                   p_estadoVeiculo ESTADO_VEICULO.DESIGNACAO%type)
AS
    idEstadoVeiculo ESTADO_VEICULO.ID_ESTADOVEICULO%type;
    counter         integer;
BEGIN
    ...
    -- Get id estado veiculo atraves da designacao
    Select COUNT(ID_ESTADOVEICULO) INTO counter FROM ESTADO_VEICULO WHERE upper(DESIGNACAO) = upper(p_estadoVeiculo);
    ...
    IF counter > 0 then
        ...
        UPDATE DRONE
        SET ID_ESTADO = idEstadoVeiculo
        WHERE NUMREGISTO = p_nrRegisto;
    else
        raise_application_error(-20001, 'Estado Drone inválido');
    end if;

END;
/

create PROCEDURE prcRemoverDrone(p_nrRegisto drone.NUMREGISTO%type)
AS
...
BEGIN
    ...
    -- Colocar id em variavel temporaria
    Select ID_DRONE into idVeiculo from DRONE where upper(NUMREGISTO) = upper(p_nrRegisto);

    -- Eliminar drone
    DELETE FROM DRONE WHERE upper(NUMREGISTO) = upper(p_nrRegisto);

    -- Eliminar veiculo
    DELETE FROM veiculo WHERE ID_VEICULO = idVeiculo;
...
create PROCEDURE prcadicionarcreditos(p_idcliente conta.id_cliente%TYPE,
                                      p_numcreditos INTEGER) AS
BEGIN
    UPDATE conta
    SET credito = credito + p_numcreditos
